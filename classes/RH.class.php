...
class RH extends Livres {
    ...
    public function listarPessoas($id = "") {
        ...
        $formacoes = $this->formacoesPorPessoa();
        $listaFormacoes = $this->listarFormacoes();
        ...
        foreach ($rs as $row) {
            foreach ($listaFormacoes as $formacao) {
                if (is_array($formacoes)) {
                    if (array_key_exists($row["id"], $formacoes) && array_key_exists($formacao["id"], $formacoes[$row["id"]])) {
                        $dt = $formacoes[$row["id"]][$formacao["id"]];
                    } else {
                        $dt = 0;
                    }
                } else {
                ...
                $return[$row["id"]]["nome"] = $row["nome"];
                $return[$row["id"]]["id"] = $row["id"];
                $return[$row["id"]]["formacoes"][$formacao["id"]] = $dt;
            }
        }
        // echo "<pre>";
        // print_r($return);
    ...
    public function cadastraFormacao($id_admin, $id_formacao, $data) {
        if (!$this->listarPessoas($id_admin)) {
            return false;
        }
        if (!$this->listarFormacoes($id_formacao)) {
            return false;
        }
        if ($this->formacoesPorPessoa($id_admin, $id_formacao)) {
            $sql = "UPDATE rhControleFormacao SET data = ? WHERE id_admin = ? and id_formacao = ?";
        } else {
            $sql = "INSERT INTO rhControleFormacao (data, id_admin, id_formacao) VALUES (?, ?, ?)";
        }
        $st = $this->conn()->prepare($sql);
        if ($st->execute([$data, $id_admin, $id_formacao])) {
            return true;
        } else {
            return false;
        }
    ...
    public function listarFormacoes($filter = "") {
        $sql = "SELECT * FROM rhFormacoes";
        if ($filter != "") {
            $sql .= " WHERE ";
            if (is_numeric($filter)) {
                $sql .= "id = ".$filter;
            } else {
                $sql .= "descricao LIKE '%".$filter."%'";
            }
        }
    ...
    public function formacoesPorPessoa($filterPessoa = "", $filterFormacao = "") {
        $sql = "SELECT * FROM rhControleFormacao";
        $where = "";
        if ($filterPessoa != "") {
            $where = "id_admin = ".$filterPessoa;
        }
        if ($filterFormacao != "") {
            if ($where != "") $where .= " AND ";
            $where = "id_formacao = ".$filterFormacao;
        }
        ...
        foreach ($rs as $row) {
            $return[$row["id_admin"]][$row["id_formacao"]] = $row["data"];
        }
