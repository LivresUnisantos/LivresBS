...
<?php
    ...
    */
	...
	$sql = "SELECT * FROM PedidosVar WHERE idConsumidor = ".$idConsumidor." AND (idOpcao1 IS NOT NULL or idOpcao2 IS NOT NULL) AND idCalendario = ".$idProximaEntrega;
	$st = $conn->prepare($sql);
	$st->execute();
            ...
            if (strlen($cpf) == 0 || ($opcao1 == "NULL" && $opcao2 == "NULL")) {
                ...
                if (!isset($_POST["adicional"])) {
                    ...
                    if (strlen($_POST["delivery"]) == 0) {
                    	...
                    	$st->execute();
                    	if ($st->rowCount() > 0) {
                    	    $sql = "UPDATE PedidosVar SET idOpcao1 = ".$opcao1.", idOpcao2 = ".$opcao2.", adicional = ".$adicional.", delivery='".$delivery."' WHERE idConsumidor = ".$idConsumidor." AND idCalendario = ".$idProximaEntrega;
                    	    $st = $conn->prepare($sql);
            	            $st->execute();
            	            setLog("log.txt","CPF: ".$cpf." - Pedido atualizado (já havia resposta Livres)",$sql);
                    	} else {
                            $sql = "INSERT INTO PedidosVar (idConsumidor,idCalendario,idOpcao1,idOpcao2,adicional,delivery) VALUES (".$idConsumidor.",".$idProximaEntrega.",".$opcao1.",".$opcao2.",$adicional,'".$delivery."')";
                            $st = $conn->prepare($sql);
            	            $st->execute();
