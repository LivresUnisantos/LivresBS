	...
	<p></p>
		<div class="grid-container">
                    ...
                    <div class="grid-x grid-padding-x">
				        <div class="large-12 medium-12 cell text-center callout" id="semanal">
                            ...
                            <div class="text-left callout">
								...
								<?php echo '<br>Cesta fixa: R$'.number_format($row["pedido_fixa"],2,",","."); ?>
								...
								<?php if ($row["pedido_mensal"] > 0) { echo '<br>+ Mensal: '.number_format($row["pedido_mensal"],2,",","."); } ?>
                            </div>
					    </div>
					</div>
                    <?php
            ...
            <?php
    			...
    			<div class="grid-x grid-padding-x">
                    <?php
                    $sql = "SELECT * FROM Pedidos LEFT JOIN produtos ON Pedidos.IDProduto = produtos.id WHERE produtos.previsao <= '".date('Y-m-d')."' AND Pedidos.IDConsumidor = ".$idConsumidor." AND Pedidos.Frequencia = 'Semanal' AND Pedidos.Quantidade > 0 ORDER BY produtos.nome";
                    $st = $conn->prepare($sql);
                    $st->execute();
                    if ($st->rowCount() > 0) {
                    ?>
                <! -- CESTA IMEDIATA SEMANAL -->
    				<div class="large-12 medium-12 cell text-center callout" id="semanal">
    			...
    			<! -- CESTA IMEDIATA QUINZENAL -->
    				<div class="large-12 medium-12 text-center callout" id="quinzenal">
    					...
    					<div class="grid-x grid-padding-x">
    						...
    						$sql = "SELECT * FROM Pedidos LEFT JOIN produtos ON Pedidos.IDProduto = produtos.id WHERE produtos.previsao <= '".date('Y-m-d')."' AND Pedidos.IDConsumidor = ".$idConsumidor." AND Pedidos.Quantidade > 0 AND (Pedidos.Frequencia = 'Semanal' OR Pedidos.Frequencia = 'Quinzenal') ORDER BY produtos.nome";
    						$st = $conn->prepare($sql);
    						$st->execute();
                ...
                <?php
                $sql = "SELECT * FROM Pedidos LEFT JOIN produtos ON Pedidos.IDProduto = produtos.id WHERE produtos.previsao <= '".date('Y-m-d')."' AND Pedidos.IDConsumidor = ".$idConsumidor." AND Pedidos.Quantidade > 0 AND (Pedidos.Frequencia = 'Mensal') ORDER BY produtos.nome";
                $st = $conn->prepare($sql);
                $st->execute();
    
                if ($st->rowCount() > 0) {
                ?>
                <! -- CESTA IMEDIATA MENSAL -->
    			<div class="grid-x grid-padding-x">
        			<div class="medium-1">&nbsp;</div>
        			<div class="large-12 medium-12 text-center callout" id="quinzenal">
        				<h5>CESTA IMEDIATA MENSAL</h5>
        				<div class="grid-x grid-padding-x">
        					<?php
    			...
    			<div class="grid-x grid-padding-x">
        			<div class="medium-1">&nbsp;</div>
        			<div class="large-12 medium-12 text-center callout" id="quinzenal">
        				<h5>CESTA COMPROMISSO MENSAL</h5>
        				<div class="grid-x grid-padding-x">
        					<?php
        					//Loop de pedidos mensais
        					$sql = "SELECT * FROM Pedidos LEFT JOIN produtos ON Pedidos.IDProduto = produtos.id WHERE Pedidos.IDConsumidor = ".$idConsumidor." AND Pedidos.Quantidade > 0 AND (Pedidos.Frequencia = 'Mensal') ORDER BY produtos.nome";
