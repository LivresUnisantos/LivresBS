...
$conn = new PDO($dsn, $user, $password,
    ...
    // esse sql verifica se existe pedidos neste cpf
    $sql = "select * from pedidos_consolidados pd inner join Consumidores cons on cons.id = pd.consumidor_id WHERE cons.cpf = '".$cpf."'";
    $st = $conn->prepare($sql);
    $st->execute();

  	$senha = "SELECT * FROM Consumidores WHERE cpf='".$cpf."'";

    $checksenha = $conn->prepare($senha);
    $checksenha->execute();


/// aqui verifica se existe senha
		$result = $checksenha->fetch(PDO::FETCH_OBJ);
...
<div class="grid-container">
		<div class="row justify-content-center ">
			<div class="large-4 " style='margin-top: 10%; margin-left: 50%'>
				Usuário com senha não registrada, digite uma senha para cadastrar
				<form action="" method="POST">
					<br>
					<input style="width: 45%" type="text"  placeholder="Digite uma senha" value="" name="criarSenha" id="criarSenha" />
					<input style="width: 45%" type="text"  placeholder="Confirme a senha" value="" name="confirmarSenha" id="confirmarSenha" />
					<input type="submit" value="Enviar" />
				</form>
			</div>
...
<div class="grid-container">
		<div class="row justify-content-center ">
			<div class="large-4 " style='margin-top: 10%; padding-left:20%; '>
			Digite sua senha
				<form action="" method="POST">
					<br>

					<input style="width: 75%" type="text" value="<?php echo $cpf ?>"  name="cpf" id="cpf" />

					<input style="width: 75%" type="text"  placeholder="Senha" value="" name="senha" id="senha" />

					<input type="submit" name="senhaEnviar" value="Enviar" />
				</form>
			</div>
...
if(isset($_POST['senhaEnviar'])){

  ///////////// fim verifica

   $senha = $_POST["senha"];
		$login = "SELECT * FROM Consumidores WHERE cpf='".$cpf."' AND senha='".$senha."'";

    $logincomSenha = $conn->prepare($login);
    $logincomSenha->execute();

							if($logincomSenha->rowCount() == 1)
								{
								 if ($st->rowCount() == 0) {
								 	echo
"<script> $('body').empty()</script>"; echo "Não há nenhuma entrega
prevista!";
exit();
}
  		...
  		<div class="grid-container">
  			<div class="grid-x grid-padding-x callout">
  				<div class="large-12 cell">
  				    <a href="myIndex.php">Consultar outro consumidor</a>
...
<?php
  ...
  exit();
	...
	echo "<div style='width: 56%;' class='alert alert-danger mt-5 text-center' >Senha inválida!</div>";
	echo "</div>";
	exit();
}
}
