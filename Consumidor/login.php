...
if (isset($_POST["tipo"])) {
    ...
    if ($_POST["tipo"] == "login") {
        ...
        if (strpos($login, '@') === false) {
            //sem @ no login, usuário logando com cpf
        ...
        if (strlen($login) > 0 && strlen($senha) > 0) {
            ...
            if ($st->rowCount() == 1) {
                ...
                if (password_verify($senha,$senhaDB)) {
                    ...
                    setLog("log.txt","Usuário logado. CPF: ".$rs[0]["cpf"]." / Email: ".$rs[0]["email"],"");
        			echo '<script>window.location = "'.$origem.'";</script>';
                } else {
                    $alerta = "CPF/Senha incorreto(s).";
                    setLog("log.txt",$login." tentou logar com cpf/senha incorreto","");
                }
            } else {
                $alerta = "Seu cadastro não existe, clique no botão 'cadastre-se' abaixo'! Verifique que digitou seu cpf ou email corretamente no login.";
                setLog("log.txt",$login." tentou logar com cpf inexistente","");
            }
        } else {
	...
	<div class="container">
		<div class="form">
			...
			<form class="register-form" method="POST" name="registro" id="registro" action="">
				...
				if (isset($_SESSION["grupo"])) {
    				...
    				echo '<p><select id="grupo" name="grupo" data-validation="length" data-validation-length="min1">
    				    ...
    				    <option value="pre-comunidade">Pré-comunidade</option>
			...
			<form class="login-form" id="login" name="login" method="POST" action="">
				...
				<p><label for="usuario">CPF ou Email</label><input type="text" placeholder="CPF ou Email" name="usuario" id="usuarioL" /></p>
