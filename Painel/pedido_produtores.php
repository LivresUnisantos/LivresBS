...
if (isset($_GET["data"])) {
	if (strlen($_GET["data"]) > 0) {
		...
		foreach ($rs as $row) {
		    ...
		    $produtos[$row["id"]]["unidade"] = $row["unidade2"];
		    $produtos[$row["id"]]["preco"] = $row["preco"]*$row["multiplicador_unidade2"];
		    $produtos[$row["id"]]["preco_produtor"] = $row["preco_produtor"]*$row["multiplicador_unidade2"];
		    $produtos[$row["id"]]["multiplicador"] = $row["multiplicador_unidade2"];
		}
		//Pedido Fixo
		$sql = getSQLPedidoSemana($dataEntrega,$frequencia,1);
		$st = $conn->prepare($sql);
		$st->execute();
		$rs=$st->fetchAll();
		$pedidoFixo=[];
		foreach ($rs as $row) {
		    ...
		    $pedidoFixo[$row["produtor"]][$row["IDProduto"]]["quantidade"]+=$row["Quantidade"]/$row["multiplicador_unidade2"];
		    $pedidoFixo[$row["produtor"]][$row["IDProduto"]]["subtotal"]+=$row["Quantidade"]*$row["preco_produtor"];
		    $pedidoFixo[$row["produtor"]][$row["IDProduto"]]["unidade"]=$row["unidade2"];
		    $pedidoFixo[$row["produtor"]][$row["IDProduto"]]["produto"]=$row["nome"];
		}
		//Pedidos Variáveis
		$sql = "SELECT produtos.id, produtos.nome, produtos.unidade2, produtos.preco_produtor, produtosVar.Quantidade, produtos.multiplicador_unidade2,  ";
		...
		if ($st->rowCount() > 0) {
		    ...
		    foreach ($rs as $row) {
		        if ($row["Quantidade"] > 0) {
    		        ...
    		        $pedidoVariavel[$row["produtor"]][$row["id"]]["unidade"] = $row["unidade2"];
    		        $pedidoVariavel[$row["produtor"]][$row["id"]]["quantidade"] = $row["Quantidade"]/$row["multiplicador_unidade2"];
    		        $pedidoVariavel[$row["produtor"]][$row["id"]]["preco"] = $row["preco_produtor"]*$row["multiplicador_unidade2"];
    		        $pedidoVariavel[$row["produtor"]][$row["id"]]["subtotal"] = $row["preco_produtor"]*$row["Quantidade"]/$row["multiplicador_unidade2"];
		        }
		    }
		}
		//Pedido Completo
