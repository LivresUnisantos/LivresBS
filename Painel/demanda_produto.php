...
echo '<td rowspan="2">Total Pedido</td>';
echo '</tr>';
echo '<tr class="firstLine">';
...
echo '<td>G1S + G2Q + G3S + G4S + G5Q</td>';
echo '<td>G1Q + G2S + G3Q + G4Q + G5S</td>';
for ($i = 1; $i <= $totalGrupos; $i++) {
    echo '<td>G'.$i.'</td>';
}
/*
...
foreach ($produtos as $idProduto=>$produto) {
    ...
    //por entrega
    echo '<td>'.($produto["quantidade"]["Semanal"][1]+$produto["quantidade"]["Quinzenal"][2]+$produto["quantidade"]["Semanal"][2]+$produto["quantidade"]["Semanal"][3]+$produto["quantidade"]["Semanal"][4]+$produto["quantidade"]["Semanal"][5]+$produto["quantidade"]["Quinzenal"][5]).'</td>';
    echo '<td>'.($produto["quantidade"]["Quinzenal"][1]+$produto["quantidade"]["Semanal"][1]+$produto["quantidade"]["Semanal"][2]+$produto["quantidade"]["Quinzenal"][3]+$produto["quantidade"]["Semanal"][3]+$produto["quantidade"]["Semanal"][4]+$produto["quantidade"]["Quinzenal"][4]+$produto["quantidade"]["Semanal"][5]).'</td>';
    ...
    $somaTemp = 0;
    for ($i = 1; $i <= $totalGrupos; $i++) {
        $somaTemp += $produto["quantidade"]["Semanal"][$i]*4 + $produto["quantidade"]["Quinzenal"][$i]*2;
        $somaTemp += $produto["quantidade"]["Mensal"][$i];
    }
    if ($somaTemp > 6) {
        echo '<td style="background:#ffff00;">'.$somaTemp.'</td>';
    } else {
        echo '<td>'.$somaTemp.'</td>';
    }
    echo "</tr>";
