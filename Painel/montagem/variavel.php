...
$Pager = new Pager("index.php?lbs=variavel&page=", "<<", ">>", 5);
...
<div class="box box100">
    <div class="panel j_montagem" style="padding: 0;">
        ...
        if (!$itensPD):
            ...
            $Read->FullRead("SELECT * FROM produtos");
            $ReadProdutos = $Read->getResult();
            foreach ($ReadProdutos as $produto) {
                $listaProdutos[$produto["id"]] = $produto;
            }
            
            $data=date("Y-m-d H:i:s", strtotime($StartDate));
            $Read->FullRead("SELECT * FROM PedidosVar ped LEFT JOIN Calendario cal on ped.idCalendario = cal.id WHERE cal.data = '".$data."'");
            $ReadRespostas = $Read->getResult();
            foreach ($ReadRespostas as $resposta) {
                $respostas[$resposta["idConsumidor"]] = $resposta;
            }
            $i = $Pager->getOffset();
            foreach ($itensPD as $Itens):
                ...
                $Read->FullRead("SELECT * FROM " . DB_PD_CONS_ITENS . " AS a "
                        ...
                        . "ON a.item_tipo = e.id "
                        . "WHERE a.pedido_id = :pi AND a.item_tipo_cesta = 'variavel'"
                        . "ORDER BY d.nome, a.item_id", "pi={$pedido_id}");
                $ReadPrincipal = $Read->getResult();
                require 'var_cesta.php';
                echo $cesta_montagem;
            endforeach;
