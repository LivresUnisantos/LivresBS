...
<link rel="stylesheet" href="painel.css">
    ...
    <script>
        function ativar(destino,nivel) {
            if (nivel < 10000) {
                alert('Você não possui permissão para executar essa ação');
            } else {
                var r = confirm("Deseja realmente ativar o consumidor?");
                if (r == true) {
                    window.location.href= destino;
                }
            }
        }
        function desativar(destino,nivel) {
            if (nivel < 10000) {
                alert('Você não possui permissão para executar essa ação');
            } else {
                var r = confirm("Deseja realmente desativar o consumidor?");
                if (r == true) {
                    window.location.href= destino;
                }
            }
...
foreach ($consumidores as $consumidor=>$dados) {
    ...
    if ($dados["ativo"] == 0) {
        echo '<td><a href="javascript:ativar(\'?cpf='.$dados["cpf"].'&ativo=1\','.$_SESSION["level"].')">Ativar</a></td>';
    } else {
        echo "ja ativo";
    }
    echo '</tr>';
