...
class Session {
    ...
    private function sessionStart() {
        ...
        if (isset($_SERVER['HTTP_REFERER']) && !empty($_SERVER['HTTP_REFERER'])):
            $this->Session['online_referer'] = filter_var($_SERVER['HTTP_REFERER'], FILTER_DEFAULT);
        endif;
        ...
        $Update = new Update;
        $Read = new Read;
        $Read->ExeRead(DB_VIEWS_ONLINE, "WHERE online_ip = :ses", "ses={$this->Session['online_ip']}");
        if ($Read->getResult()):
            $this->Session['online_startview'] = $Read->getResult()[0]['online_startview'];
            $this->Session['online_ip'] = $Read->getResult()[0]['online_ip'];
            $this->Session['online_agent'] = $Read->getResult()[0]['online_agent'];
            $this->Session['online_referer'] = $Read->getResult()[0]['online_referer'];
            $Update->ExeUpdate(DB_VIEWS_ONLINE, $this->Session, "WHERE online_ip = :id", "id={$this->Session['online_ip']}");
        else:
    ...
    private function viewsStart() {
        $Read = new Read;
        $Read->ExeRead(DB_VIEWS_VIEWS, "WHERE views_date = CURDATE()");
        if ($Read->getResult()):
            ...
            $Update = new Update;
            $Update->ExeUpdate(DB_VIEWS_VIEWS, $UpdateView, "WHERE views_date = CURDATE() AND views_id >= :id", "id=1");
